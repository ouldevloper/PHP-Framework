...
class MultiFileUpload{
	...
	private function fullname(){
		for($i = 0 ; $i < $this->files_count ; $i++) {
			...
			$this->files_name[] = str_replace(['\\','/','.',':','<','>','?','*','"','|'],'',substr(password_hash($tmp.$sessionid.time(),PASSWORD_DEFAULT),0,UPLOAD_FILE_MAX_LENGHT)).'.'.$this->file_extentions[$i];
		}
	}
	...
	private function getProperLocation(){
		...
		for ($i=0; $i < $this->files_count; $i++) { 
			...
			$dir = scandir(SAVE_UPLOAD_PATH);
			if(!in_array($tmp, $dir)){
				mkdir(SAVE_UPLOAD_PATH.$tmp,777);
			}
			$this->files_proper_location[$i] = str_replace('\\','/',SAVE_UPLOAD_PATH . $tmp.'/');
		}
	}
	...
	public function upload(){
		if ($this->isAllowedExtention() && 
			...
			!$this->hasErrors()){
			for ($i=0; $i < $this->files_count ; $i++) {
				$path = $this->files_proper_location[$i].$this->files_name[$i]; 
